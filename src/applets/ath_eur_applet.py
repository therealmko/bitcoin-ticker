...
class ath_eur_applet(BaseApplet):
    ...
    async def draw(self):
        ...
        self.screen_manager.draw_centered_text(f"€{int(ath_price_eur):,}", y_offset=-10) # Euro symbol
        ...
        if current_price_eur is not None:
            ...
            try:
                ...
                self.screen_manager.draw_centered_text(f"Now: €{int(current_price_eur):,} (ATH Zero)", scale=2, y_offset=60, # Euro symbol
                ...
                self.screen_manager.draw_centered_text(f"Now: €{int(current_price_eur):,} (Error %)", scale=2, y_offset=60, # Euro symbol
                                                     color=self.screen_manager.theme['NEGATIVE_COLOR'])
        else:
