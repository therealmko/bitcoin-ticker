...
class ath_eur_applet(BaseApplet):
    ...
    """
    TTL = const(120) # Same TTL as bitcoin_eur_applet for current price
    ...
    def __init__(self, screen_manager: ScreenManager, data_manager: DataManager):
        ...
        # Need current price data, use the BTCEUR endpoint
    ...
    def _load_ath_data(self):
        """Load ATH data from the JSON file (expected to be bitcoin_ath_eur.json)."""
        try:
            with open("bitcoin_ath_eur.json", "r") as f:
                self.ath_data = json.load(f)
                print(f"[ath_eur_applet] Loaded ATH EUR data: {self.ath_data}")
        except OSError as e:
            if e.args[0] == uerrno.ENOENT:
                print("[ath_eur_applet] bitcoin_ath_eur.json not found.")
            else:
                print(f"[ath_eur_applet] Error loading bitcoin_ath_eur.json: {e}")
            self.ath_data = None # Ensure it's None on error
        except ValueError:
            print("[ath_eur_applet] Error parsing bitcoin_ath_eur.json.")
            self.ath_data = None
        except Exception as e:
            print(f"[ath_eur_applet] Unexpected error loading ATH EUR data: {e}")
            self.ath_data = None
    ...
    async def draw(self):
        ...
        if not self.ath_data or self.ath_data.get("ath_eur") is None: # Check for 'ath_eur'
            self.screen_manager.draw_centered_text("ATH EUR Data N/A", scale=3, y_offset=0)
            gc.collect()
            return

        ath_price = self.ath_data["ath_eur"] # Use 'ath_eur'
        ath_date_str = self.ath_data.get("ath_date_eur", "Unknown date") # Use 'ath_date_eur'
        ...
        self.screen_manager.draw_centered_text(f"€{int(ath_price):,}", y_offset=-10) # Euro symbol
            ...
            if isinstance(price_data, dict):
                ...
                if price_str is not None:
                    try:
                        ...
                        print(f"[ath_eur_applet] Error converting current price: {price_str}")
        ...
        if current_price is not None:
            try:
                ...
                combined_text = f"Now: €{int(current_price):,} ({percentage_diff:+.2f}% vs ATH)" # Euro symbol
                ...
                self.screen_manager.draw_centered_text(f"Now: €{int(current_price):,} (ATH Zero)", scale=2, y_offset=60, # Euro symbol
                ...
                self.screen_manager.draw_centered_text(f"Now: €{int(current_price):,} (Error %)", scale=2, y_offset=60, # Euro symbol
                                                     color=self.screen_manager.theme['NEGATIVE_COLOR'])
        else:
